...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        if self.config.dec_ipt:
            ...
            self.ipt_blk5 = DBlock(
                2**10 * 3 if self.split else 3, [N_dec_ipt, channels[0] // 8][ipt_cha_opt], inter_channels=ic
            )
            self.ipt_blk4 = DBlock(
                2**8 * 3 if self.split else 3, [N_dec_ipt, channels[0] // 8][ipt_cha_opt], inter_channels=ic
            )
            self.ipt_blk3 = DBlock(
                2**6 * 3 if self.split else 3, [N_dec_ipt, channels[1] // 8][ipt_cha_opt], inter_channels=ic
            )
            self.ipt_blk2 = DBlock(
                2**4 * 3 if self.split else 3, [N_dec_ipt, channels[2] // 8][ipt_cha_opt], inter_channels=ic
            )
            self.ipt_blk1 = DBlock(
                2**0 * 3 if self.split else 3, [N_dec_ipt, channels[3] // 8][ipt_cha_opt], inter_channels=ic
            )
        else:
            self.split = None

        self.decoder_block4 = DecoderBlock(
            channels[0] + ([N_dec_ipt, channels[0] // 8][ipt_cha_opt] if self.config.dec_ipt else 0), channels[1]
        )
        ...
        self.decoder_block1 = DecoderBlock(
            channels[3] + ([N_dec_ipt, channels[2] // 8][ipt_cha_opt] if self.config.dec_ipt else 0), channels[3] // 2
        )
        self.conv_out1 = nn.Sequential(
            nn.Conv2d(
                channels[3] // 2 + ([N_dec_ipt, channels[3] // 8][ipt_cha_opt] if self.config.dec_ipt else 0),
                1,
                1,
                1,
                0,
    ...
    def forward(self, features):
        ...
        if self.config.dec_ipt:
            patches_batch = (
                image2patches(x, patch_ref=x4, transformation="b c (hg h) (wg w) -> b (c hg wg) h w")
                if self.split
                else x
            )
            x4 = torch.cat(
                (
                    x4,
                    self.ipt_blk5(F.interpolate(patches_batch, size=x4.shape[2:], mode="bilinear", align_corners=True)),
                ),
                1,
            )
        ...
        if self.config.dec_ipt:
            patches_batch = (
                image2patches(x, patch_ref=_p3, transformation="b c (hg h) (wg w) -> b (c hg wg) h w")
                if self.split
                else x
            )
            _p3 = torch.cat(
                (
                    _p3,
                    self.ipt_blk4(F.interpolate(patches_batch, size=x3.shape[2:], mode="bilinear", align_corners=True)),
                ),
                1,
            )
        ...
        if self.config.dec_ipt:
            patches_batch = (
                image2patches(x, patch_ref=_p2, transformation="b c (hg h) (wg w) -> b (c hg wg) h w")
                if self.split
                else x
            )
            _p2 = torch.cat(
                (
                    _p2,
                    self.ipt_blk3(F.interpolate(patches_batch, size=x2.shape[2:], mode="bilinear", align_corners=True)),
                ),
                1,
            )
        ...
        if self.config.dec_ipt:
            patches_batch = (
                image2patches(x, patch_ref=_p1, transformation="b c (hg h) (wg w) -> b (c hg wg) h w")
                if self.split
                else x
            )
            _p1 = torch.cat(
                (
                    _p1,
                    self.ipt_blk2(F.interpolate(patches_batch, size=x1.shape[2:], mode="bilinear", align_corners=True)),
                ),
                1,
            )
        ...
        if self.config.dec_ipt:
            ...
            _p1 = torch.cat(
                (
                    _p1,
                    self.ipt_blk1(F.interpolate(patches_batch, size=x.shape[2:], mode="bilinear", align_corners=True)),
                ),
                1,
            )
...
class BiRefNetC2F(
    ...
    def forward(self, x):
        ...
        pred = F.interpolate(
            scaled_preds[-1] if not (self.config.out_ref and self.training) else scaled_preds[1][-1],
            size=x_ori.shape[2:],
            mode="bilinear",
            align_corners=True,
        )
        ...
        if self.training:
            if self.config.out_ref:
                ...
                return [
                    ([outs_gdt_pred + outs_gdt_pred_HR, outs_gdt_label + outs_gdt_label_HR], outs + outs_HR),
                    class_preds_lst,
                ]  # handle gt here
            else:
                return [
